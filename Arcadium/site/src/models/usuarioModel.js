...
function autenticar(credenciais) {
    ...
    var instrucao = `
        SELECT email, senha FROM usuario WHERE email = '${credenciais.email}' AND senha = '${credenciais.senha}';
    `;
...
function cadastrar(credenciais) {
    ...
    var instrucao = `
        INSERT INTO usuario (email, senha) VALUES ('${credenciais.email}', '${credenciais.senha}');
    `;
    console.log("Executando a instrução SQL: \n" + instrucao);
    return database.executar(instrucao);
}
